...
def run_simulation(x0, params, model, path, N_horizon, Tf, num_laps, max_simulation_time):
    ...
    return np.array(state_history), np.array(input_history)
...
def calculate_initial_heading(x, y, path_center_x, path_center_y, path_radius):
    ...
    # Calculate the basic angle from the path center to the point
    ...
    # Constrain the angle to [-π, π]
    return (adjusted_angle + np.pi) % (2 * np.pi) - np.pi
...
def main():
    ...
    # Define the grid
    grid_size = 20.0  # meters between grid points
    grid_extent = 20.0  # meters in each direction from center
    ...
    for x_start in x_range:
        for y_start in y_range:
            ...
            state_history, input_history = run_simulation(x0, params, model, path, N_horizon, Tf, num_laps,  max_simulation_time)
    ...
    # Save results
    save_results(all_state_histories, all_input_histories, x_range, y_range, results_dir)
    u.plot_trajectories()

def save_results(all_state_histories, all_input_histories, x_range, y_range, results_dir):
    ...
    for i, state_history in enumerate(all_state_histories):
    ...
    # Prepare data for input histories
    input_data = []
